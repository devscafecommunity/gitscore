...
export default async function ListPosts() {
  const raWposts = (await getAllPosts()) as any[];
  const posts = raWposts.filter((post) => post.properties.Public.checkbox === true);
        ...
        posts.map((post, index) => {
          return (
              <PostCard 
                ...
                authorProfilePicture={getAuthorById(post.properties.Author.people[0].id).properties["Author Profile Picture"].files[0]?.external?.url || "https://devscafe.pt/assets/img/author.png"}
